...
|---|---|---|---|
|400|`INVALID_INPUT_VALUE`|필수값 누락, **금액에 소수부**, `Idempotency-Key` 헤더 누락|검증 실패 사유 (공통 핸들러가 생성)|
|400|`AMOUNT_MISMATCH`|금액 불일치 (위변조 검증 실패)|“요청된 결제 금액이 실제 주문 금액과 일치하지 않습니다.”|
|400|`INVALID_PAYMENT_STATUS`|주문이 결제 가능 상태가 아니거나 유효시간 초과|“이미 취소되었거나 유효하지 않은 결제 건입니다.”|
|401|`UNAUTHORIZED`|로그인 세션 만료 및 인증 토큰 누락|“인증이 필요합니다.”|
|402|`PAYMENT_REQUIRED`|결제 한도 초과 또는 잔액 부족 시|“결제 잔액이 부족하거나 한도를 초과했습니다.”|
|404|`ORDER_NOT_FOUND`|존재하지 않는 주문, **또는 타인 소유 주문**|“존재하지 않는 주문입니다.”|
|409|`DUPLICATE_PAYMENT_REQUEST`|Idempotency Key 충돌, **또는 같은 주문에 이미 성공한 결제 존재**|“동일한 결제 요청이 처리 중이거나 이미 완료되었습니다.”|
|500|`INTERNAL_SERVER_ERROR`|PG 승인 처리 중 네트워크 타임아웃 등 에러|“결제 승인 중 오류가 발생했습니다. 다시 시도해주세요.”|

> **404 `ORDER_NOT_FOUND`는 검토 과정에서 추가됐다.** 타인의 주문을 결제하려는 시도를 처리해야 하는데
> 초안 에러표에 404가 없었다. 결제 조회와 같은 논리로 **주문의 존재 여부도 구분해 노출하지 않는다.**
>
> **성공은 200 OK다.** 응답 본문이 생성된 리소스의 표현이 아니라 완료 통지이고 `paymentId`도 담지 않으므로
> 201을 쓰지 않는다.
>
> **재요청(같은 `Idempotency-Key`, 같은 본문)은 저장된 응답을 그대로 재생한다.** PG를 다시 호출하지 않으며
> 상태 코드도 최초 응답과 같다.
...
| 에러 코드 | 코드 | 상태 | 메시지 |
| --- | --- | --- | --- |
| `AMOUNT_MISMATCH` | PAY4001 | 400 | 요청된 결제 금액이 실제 주문 금액과 일치하지 않습니다. |
| `INVALID_PAYMENT_STATUS` | PAY4002 | 400 | 이미 취소되었거나 유효하지 않은 결제 건입니다. |
| `PAYMENT_REQUIRED` | PAY402 | 402 | 결제 잔액이 부족하거나 한도를 초과했습니다. |
| `PAYMENT_NOT_FOUND` | PAY404 | 404 | 존재하지 않는 결제 내역입니다. |
| `ORDER_NOT_FOUND` | PAY4041 | 404 | 존재하지 않는 주문입니다. |
| `DUPLICATE_PAYMENT_REQUEST` | PAY409 | 409 | 동일한 결제 요청이 처리 중이거나 이미 완료되었습니다. |

- 400·404로 응답할 사유가 각각 둘이라 코드에 일련번호를 붙였다. 코드 문자열은 로그·추적용이며
  응답의 `error` 필드에는 enum 이름이 실린다.
