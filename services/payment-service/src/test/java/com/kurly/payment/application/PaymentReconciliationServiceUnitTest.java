...
class PaymentReconciliationServiceUnitTest {
    ...
    private void givenTarget(PaymentStatus status) {
        given(paymentRecordService.claimReconcilable(anyInt(), any()))
    ...
    class ClaimTest {
        ...
        void 유예_시간을_뺀_시각을_기준으로_대상을_찾는다() {
            ...
            given(paymentRecordService.claimReconcilable(anyInt(), any())).willReturn(List.of());
            LocalDateTime before = LocalDateTime.now().minus(STALE_AFTER);

            reconcile();

            ArgumentCaptor<LocalDateTime> captor = ArgumentCaptor.forClass(LocalDateTime.class);
            verify(paymentRecordService).claimReconcilable(eq(100), captor.capture());
    ...
    class FailureTest {
        ...
        void 한_건이_실패해도_나머지를_계속_대사한다() {
            given(paymentRecordService.claimReconcilable(anyInt(), any())).willReturn(List.of(
                    new PaymentRecordService.ReconcileTarget(PAYMENT_ID, ORDER_ID, AMOUNT, PaymentStatus.REQUESTED),
                    new PaymentRecordService.ReconcileTarget(11L, 222L, AMOUNT, PaymentStatus.REQUESTED)));
            willThrow(new IllegalStateException("PG timeout")).given(pgClient).findByOrderId(ORDER_ID);
