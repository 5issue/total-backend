...
| 2026-09-22 | OMS가 배송지 주소의 권역(region)에 따라 물류 창고를 배정할 수 있도록, `GET /internal/v1/wms/warehouses` 조회 API를 api-spec에 먼저 작성(Java 구현 전, 문서 우선). 신규 `models/warehouses.dto.tsp`(`WarehouseResponse`)/`routes/internal/warehouses.internal.tsp`. `region`은 `Warehouse` 엔티티에 아직 없는 제안 단계 필드 — 우편번호 구간 매핑인지 시/도 고정 목록인지 자유 코드인지 체계가 미확정이라 우선 nullable 자유 문자열로 설계하고 `@doc`에 propose 단계임을 명시. 창고 수가 적어 페이지네이션 없이 배열 그대로 반환(`LocationResponse[]`와 동일 패턴) | 완료 | `npm run build/lint` 통과(경고 25→26, 신규 스키마 1개분 — 기존 패턴과 동일), `sync:notion:dry`로 엔드포인트 21→22개 확인 및 `@doc`의 propose 경고 문구가 페이지 개요에 정상 렌더링되는 것 확인. Java 구현(엔티티 컬럼 추가 마이그레이션 포함)은 region 값 체계가 OMS/운영 측과 확정된 뒤로 보류 — erd-spec 논의사항에 8번으로 기록 |
| 2026-09-22 | 바로 위 `region` 필드를 자유 문자열 대신 시/도 단위 고정 Enum(17개 광역자치단체)으로 확정 — `com.kurly.wms.domain.enums.Region`(StorageType과 동일하게 한글 label 포함) 신규, `V10__add_warehouse_region.sql`(컬럼 추가 + `chk_warehouse_region` CHECK 제약으로 DB 레벨에서도 값 강제), `Warehouse` 엔티티에 `@Enumerated(EnumType.STRING)` region 필드 추가. api-spec `models/warehouses.dto.tsp`에도 `Region` enum 신설, 쿼리 파라미터를 `string`에서 `Region`으로 좁힘. `seed_wms_warehouse.sql`의 8개 창고 INSERT에 실제 주소 기준 region 값 매핑(김포/평택/안산=GYEONGGI, 창원=GYEONGNAM, 컬리나우 4곳=SEOUL) — 이미 시드가 적용된 기존 DB에도 값이 채워지도록 `ON CONFLICT (code) DO NOTHING`을 `DO UPDATE SET region = EXCLUDED.region`으로 변경(다른 컬럼은 안 건드림) | 완료 | `./gradlew :common:build :wms-service:build` 통과. 실제 서버로 Flyway V10 적용 확인 후 시드 재실행 — 이미 존재하던 8개 창고 전부 region이 정확히 채워지는 것 확인(김포/평택/안산 GYEONGGI, 창원 GYEONGNAM, 컬리나우 4곳 SEOUL), CHECK 제약이 잘못된 값(`INVALID_REGION`)을 실제로 거부하는 것도 확인. `npm run build/lint` 통과, 생성된 OpenAPI에 `Warehouses.Region` enum이 17개 값 그대로 반영된 것 확인. erd-spec 논의사항 8번을 해결됨으로 갱신 — 다만 `GET /internal/v1/wms/warehouses`의 실제 컨트롤러/서비스 Java 구현은 아직 없음(계속 api-spec만 작성된 상태) |
| 2026-09-22 | 사용자 질문으로 "한 창고가 여러 권역을 담당할 수 있지 않냐"는 지적을 받고, 바로 위에서 만든 `warehouse.region` 단일 컬럼 설계를 1:N `warehouse_region` 테이블로 재설계 — `Warehouse.region: Region`(단일) → `Warehouse.regions: Set<Region>`(`@ElementCollection`+`@CollectionTable(name="warehouse_region")`)으로 변경. `V10__add_warehouse_region.sql`이 아직 git에 커밋되지 않은 상태였어서(이번 세션에서 방금 추가됨) V11을 새로 쌓는 대신 V10 파일 자체를 고쳐썼다 — 로컬 dev DB에는 이미 구버전 V10이 적용돼 있어서, 그 컬럼/제약을 수동으로 되돌리고 `flyway_schema_history`의 V10 행을 지운 뒤 재부팅해 새 버전을 깨끗하게 재적용했다(팀과 공유되지 않은 로컬 전용 마이그레이션이라 가능한 방법 — 이미 공유됐다면 V11로 쌓았을 것). api-spec `WarehouseResponse.region: Region \| null` → `regions: Region[]`로 변경. `seed_wms_warehouse.sql`도 재작성 — warehouse INSERT에서 region 컬럼 제거, 대신 이 8개 창고분을 먼저 지우고(`DELETE FROM warehouse_region WHERE warehouse_id IN (...)`) 다시 채우는 별도 블록 추가. 실제 지리적 인접성을 참고해 3곳에 복수 권역을 부여: 김포(경기+서울+인천), 평택(경기+충남), 창원(경남+부산) — 안산과 컬리나우 4곳은 단일 권역 유지 | 완료 | 로컬 DB에서 구버전 V10 되돌리기(컬럼/제약 DROP + flyway_schema_history 행 삭제) → 재부팅해 새 V10(`warehouse_region` 테이블) 적용 확인 → 시드 재실행해 8개 창고 중 3곳이 실제로 복수 region을 갖는 것 확인(김포 GYEONGGI/INCHEON/SEOUL, 평택 CHUNGNAM/GYEONGGI, 창원 BUSAN/GYEONGNAM). 재실행 idempotency도 재확인(두 번 실행해도 정확히 12행 유지). CHECK 제약이 잘못된 값을 여전히 거부하는 것도 재확인. `./gradlew :common:build :wms-service:build`, `npm run build/lint` 통과. erd-spec 8절/ER 다이어그램(WAREHOUSE_REGION 엔티티 추가)/Enum 요약/논의사항 8번 전부 갱신 |
| 2026-09-22 | 바로 위 `warehouse_region` 조인 테이블 설계를 재검토 — "테이블까지 만들 필요 없이 enum 여러 개를 그냥 warehouse에 넣을 수 없냐"는 지적을 받고 PostgreSQL 네이티브 배열 컬럼(`warehouse.regions VARCHAR(20)[]`)으로 다시 바꿨다. `V10__add_warehouse_region.sql`이 이번에도 아직 커밋 전이라 파일을 또 고쳐썼다 — 로컬 DB에서 `warehouse_region` 테이블을 DROP하고 flyway_schema_history의 V10 행을 지운 뒤 재부팅해 새 버전(배열 컬럼 + `chk_warehouse_regions` CHECK 제약, `<@` 연산자로 "배열의 모든 원소가 17개 값 중 하나인가" 검증)을 깨끗하게 재적용. `Warehouse.regions`를 `Set<Region>`+`@ElementCollection`에서 `List<Region>`+`@JdbcTypeCode(SqlTypes.ARRAY)`+`@Enumerated(EnumType.STRING)`으로 교체(Hibernate 7의 네이티브 배열 매핑 — 이 프로젝트에서 처음 쓰는 기능이라 실제로 되는지 자신이 없어서, 임시 `TempArrayMappingTest`(`ApplicationRunner`)를 만들어 실제 서버로 write/read 왕복을 검증한 뒤 삭제했다). `seed_wms_warehouse.sql`도 다시 배열 리터럴(`ARRAY['GYEONGGI','SEOUL','INCHEON']` 등) 직접 INSERT하는 방식으로 되돌리고, `ON CONFLICT (code) DO UPDATE SET regions = EXCLUDED.regions`로 재실행 시 백필되게 함 | 완료 | 실제 서버로 Hibernate 배열 매핑 자체를 먼저 검증(가장 불확실했던 부분) — 임시 테스트로 `List.of(JEJU, GANGWON)`을 저장→재조회했을 때 정확히 `[JEJU, GANGWON]`으로 왕복되는 것 확인, `ddl-auto: validate`도 배열 컬럼을 문제없이 통과하는 것 확인. 시드 재실행 후 실제 GIMPO_DC.regions를 JPA로 읽어 `[GYEONGGI, SEOUL, INCHEON]`이 정확히 나오는 것도 재확인. CHECK 제약이 배열 안에 잘못된 값이 하나라도 섞이면 전체 UPDATE를 거부하는 것도 확인. `./gradlew :common:build :wms-service:build` 통과, 임시 테스트 클래스와 테스트 데이터 전부 정리. erd-spec의 ER 다이어그램(WAREHOUSE_REGION 엔티티 제거, WAREHOUSE에 regions 배열 필드로 흡수)/8절/Enum 요약/논의사항 8번 다시 갱신 |
| 2026-09-22 | `Region`을 시/도 단위 17개 광역자치단체에서 물류 배정에 의미 있는 8개 값(`SEOUL_METRO`/`GYEONGGI_EAST`/`GYEONGGI_WEST`/`CHUNGCHEONG`/`GANGWON`/`YEONGNAM`/`HONAM`/`JEJU`)으로 재정의(한글 label 필드는 이번엔 뺌 — 사용자 요청) — 경기도만 물류센터가 몰려 있어(김포/평택/안산) 동/서로 쪼개고 나머지는 크게 묶었다. `V10__add_warehouse_region.sql`의 CHECK 제약도 새 8개 값으로 교체(이번에도 아직 커밋 전이라 로컬 DB를 되돌리고 재적용하는 방식 그대로 반복). `seed_wms_warehouse.sql`도 새 값으로 재매핑(창원은 GYEONGNAM+BUSAN이 YEONGNAM 하나로 합쳐짐). 이어서 그동안 api-spec만 있던 `GET /internal/v1/wms/warehouses`를 드디어 구현 — `WarehouseResponse`(record, `from(Warehouse)` 정적 팩토리)/`WarehouseQueryService`(isActive 생략 시 true 기본값 적용)/`WarehouseInternalController`(`@PublicApi`, `InboundInternalController`와 동일 패턴) 신규. `WarehouseJpaRepository.search()`를 처음엔 Hibernate HQL의 `array_contains(array, element)` 함수로 짰다가, enum 파라미터를 `bytea[]`로 잘못 바인딩하는 버그(`character varying[] @> bytea[]` SQL 에러)를 실제로 만나서 네이티브 쿼리 + PostgreSQL `= ANY(...)` 연산자로 우회(파라미터도 Java enum 대신 문자열로 바인딩) | 완료 | 실제 서버로 전부 검증: ① region 값 재매핑 후 시드 재실행해 8개 창고 regions가 새 값으로 정확히 채워지는 것 확인. ② `array_contains` 버전이 실제로 500 에러(`character varying[] @> bytea[]`)를 내는 것 재현한 뒤, 네이티브 쿼리로 교체해 해결 확인. ③ 실제 HTTP로 `GET .../warehouses`(필터 없음, 8건), `?region=GYEONGGI_WEST`(김포/평택/안산 3건, GIMPO_DC가 여러 region에 걸쳐 있어도 정확히 잡힘), `?region=SEOUL_METRO`(김포+컬리나우4곳=5건, 멀티 region 창고가 양쪽 필터에 다 잡히는 것도 확인) 전부 정확히 응답. ④ `isActive` 생략 시 기본 true(비활성 창고 제외), `isActive=false` 명시 시 비활성 창고만 나오는 것도 실제로 하나를 비활성화해서 확인 후 원복. `./gradlew :common:build :wms-service:build`, `npm run build/lint` 통과. erd-spec 8절/Enum 요약/논의사항 8번을 최종 구현 상태로 갱신 |
| 2026-09-24 | 사용자가 제시한 전체 출고 플로우(①RabbitMQ OrderPaidEvent 수신 ②FEFO 할당→ALLOCATED ③Spring Event 발행(★) ④작업자 피킹 API ⑤출고 완료 API(★)) 중 ③번(OutboundOrder가 ALLOCATED 될 때 Spring Event 발행)까지만 구현. ①②는 이미 구현돼 있던 `order.inventory.confirm` 소비+FEFO 할당과 동일한 것으로 확인. 검토 중 다이어그램이 놓친 부분을 짚었다 — ALLOCATED 전이가 실제로는 두 경로(생성 즉시 전부 할당되는 경우, 처음엔 일부만 돼서 PENDING_REPLENISHMENT로 있다가 나중에 `finalizeReplenishment()`로 마지막 품목까지 채워지는 경우) 있어서, 신규 `OutboundAllocatedEvent(outboundOrderId)`를 두 지점(`createFromOrderEvent()`의 `else` 분기, `finalizeReplenishment()`의 `order.allocate()` 직후) 모두에서 발행하도록 구현 — 기존 `ReplenishmentCompletedEvent`/`PutAwayCompletedEvent`와 동일한 `application/event/` 패키지+`ApplicationEventPublisher` 관례. ④⑤(피킹 작업자 배정 API, 최종 출고 완료 API)는 이번 범위에서 명시적으로 제외 | 완료 | 실제 서버로 두 경로 모두 검증(임시 리스너, 검증 후 삭제): ① 피킹존 재고가 충분해 주문 생성과 동시에 전부 ALLOCATED되는 경우 → 즉시 이벤트 발행 확인. ② 재고가 전혀 없어 UNALLOCATED로 시작 → 실제 ASN/검수/put-away로 보관존 재고를 채워 PutAwayCompletedEvent→retryUnallocated로 PENDING_REPLENISHMENT 전환 → 그 REPLENISHMENT를 confirm해 ReplenishmentCompletedEvent→finalizeReplenishment가 마지막 품목을 ALLOCATED로 완성하는 순간에만 정확히 이벤트가 발행되는 것 확인(그 이전 UNALLOCATED/PENDING_REPLENISHMENT 단계에서는 발행 안 됨도 확인). `./gradlew :common:build :wms-service:build` 통과, 테스트 데이터 정리. erd-spec 8절에 "할당 완료 신호(OutboundAllocatedEvent)" 단락 추가, 논의사항에 9번(피킹 Task 자동 생성 소비자 + 이후 API들 미구현) 신설 |
| 2026-09-24 | 사용자가 제안한 Task 테이블(현장 작업자 배정 단위, PICKING/REPLENISHMENT 통합) 설계를 검토 후 반영 — 테이블 생성과 문서화까지만(소비자/서비스 구현은 이번 범위 아님, 커밋도 안 함). 리뷰 중 발견한 문제 하나를 사용자에게 확인 후 수정: 제안된 스키마엔 `outbound_order_id`만 있었는데, 한 주문이 FEFO로 여러 LOT/로케이션에 분할 할당되면(흔함) Task 레코드만으로는 정확히 뭘 어디서 피킹해야 하는지 알 수 없는 문제가 있었다 — api-spec에 이미 있던 `PickingTaskResponse`도 `outboundItemId` 중심으로 설계돼 있어서, `outbound_item_id`를 추가하기로 확인받고 반영(한 주문이 3개 LOT으로 나뉘면 PICKING Task도 3건). 그 외 조용히 적용한 보정: DATETIME(MySQL 표기)→TIMESTAMP(6)(PostgreSQL), `outbound_order_id`/`outbound_item_id`/`stock_movement_id`/`warehouse_id`에 실제 FK 제약 추가(이 코드베이스의 기존 관례), `task_type`/`status`에 CHECK 제약 추가, 그리고 타입별 FK 상호배타를 강제하는 `chk_task_reference`(PICKING이면 order+item만 채워지고 movement는 null이어야 하고, REPLENISHMENT면 반대) 신설. `worker_id`는 `Worker` 엔티티/테이블이 아직 없어 FK 없이 순수 컬럼으로만 둠(기존에 이미 알려진 미결 사항). `V11__create_task.sql`, `Task.java`(엔티티, 아직 서비스가 없어 상태 전이 메서드는 추가 안 함 — construction만), `TaskJpaRepository`(빈 인터페이스) 신규 | 완료 | 실제 서버로 검증: ① `ddl-auto: validate`가 배열/FK/CHECK 다 걸린 새 테이블을 문제없이 통과. ② psql로 6가지 시나리오 직접 테스트 — 유효한 PICKING/REPLENISHMENT 각각 성공, PICKING인데 outbound_item_id 누락/REPLENISHMENT인데 outbound_order_id도 같이 채움/task_type에 없는 값(BOGUS)/task_no 중복 4가지 전부 정확히 거부되는 것 확인. ③ 임시 `ApplicationRunner`로 JPA가 두 유효 행을 전부 정확한 연관관계(OutboundOrder/OutboundItem/StockMovement)로 읽어오는 것도 확인 후 삭제. `./gradlew :common:build :wms-service:build` 통과, 테스트 데이터 전부 정리. **커밋하지 않음**(사용자 지시). erd-spec에 11절(Task) 신설, ER 다이어그램/Enum 요약 갱신, 논의사항 9번 갱신 + 10번(Worker 엔티티 부재) 신설 |
| 2026-09-24 | 바로 위 Task 테이블을 사용자가 재검토 — "StockMovement가 이미 있는데 task_type(PICKING, REPLENISHMENT)이 있으면 헷갈린다"는 지적으로 Task를 피킹 전용으로 좁혔다. 실제로 REPLENISHMENT는 StockMovement가 이미 자체 상태(PENDING/IN_PROGRESS/COMPLETED/CANCELED)+확정 API를 갖고 있어서, Task로 한 번 더 감싸면 두 상태가 서로 어긋날 여지가 있었던 것 — 검토 중 내가 내부적으로 갖고 있던 것과 같은 우려였다. `task_type`/`stock_movement_id`/`chk_task_reference` 전부 제거, `outbound_order_id`/`outbound_item_id`를 NOT NULL로 전환(항상 채워짐), `outbound_item_id`에 UNIQUE 제약 추가(품목당 Task 1건 보장, 조용히 적용). `V11__create_task.sql`/`Task.java`를 다시 씀(둘 다 아직 커밋 전이라 새 버전으로 교체, 로컬 DB도 되돌리고 재적용). 이어서 실제로 요청받은 소비자 로직 구현 — `TaskService.createPickingTasks(outboundOrderId)`(OutboundItem마다 Task 하나씩 생성, `task_no_seq` 전역 시퀀스로 `TSK-PICK-<yyyyMMdd>-<seq>` 채번, 이미 Task 있는 품목은 건너뜀), `OutboundReallocationListener`에 `onOutboundAllocated()` 추가(기존 두 리스너와 같은 파일 — 전부 "OutboundOrder 상태 변화에 반응"이라는 공통 성격이라 별도 파일 안 만듦), `TaskJpaRepository`에 `existsByOutboundItemId`/`nextTaskNoSequence`(네이티브 `nextval()`) 추가 | 완료 | 실제 서버로 전체 폐루프 검증: 한 상품에 2개 LOT(3개+20개)을 서로 다른 로케이션에 시딩하고 5개 주문 → FEFO로 3+2 분할 할당 → 즉시 `OutboundAllocatedEvent` 발행 → Task 정확히 2건 생성, 각각 올바른 outbound_item_id/lot_no/location으로 매핑되는 것 확인(가장 중요한 시나리오 — outbound_item_id를 추가한 이유가 실제로 검증됨). `uk_task_outbound_item_id`가 같은 품목에 대한 중복 Task 생성 시도를 실제로 거부하는 것도 재확인. `./gradlew :common:build :wms-service:build` 통과, 테스트 데이터 정리. erd-spec 11절/ER 다이어그램/Enum 요약/논의사항 9번을 최종 구현 상태로 전면 갱신(TaskType 삭제 포함) |
| 2026-09-24 | 바로 위 Task 조회/시작/완료 API를 구현 — 이번 버전은 부분 피킹(shortage)을 빼고 항상 주문 수량 전체를 피킹한 것으로 기록하기로 결정(사용자 지시). `Task` 엔티티에 `start(workerId)`/`complete()` 두 메서드 추가(둘 다 순수 setter — 상태 전이 가능 여부 검증은 `StockMovement`와 동일하게 서비스 계층 책임). `WmsErrorCode`에 `INVALID_TASK_STATUS`(WMS4096)/`TASK_WORKER_MISMATCH`(WMS4097) 추가. `TaskJpaRepository`에 `findWithPessimisticLockById`/`search(warehouseId, status)`(둘 다 `StockMovementJpaRepository`와 동일 패턴) 추가. `TaskService`에 `list()`/`start()`/`complete()` 신규 — `start()`는 PENDING인 Task를 잠그고 `workerId` 배정+시작(별도 "배정" API 없음, Worker 엔티티가 없어 배정 이력 관리 대상도 없음), 전표가 아직 ALLOCATED면 `startPicking()`으로 같이 전이. `complete()`는 IN_PROGRESS 확인 + 배정된 워커와 일치 확인 후 `outboundItem.pick(orderedQuantity)`(항상 전체 수량)로 PICKED 전환, 전표의 모든 품목이 PICKED가 됐으면 `startPacking()`으로 전이(포장 API는 아직 없어 상태만 앞서 넘겨둠). `Inventory`는 건드리지 않음 — 실물 quantity 차감은 아직 없는 최종 출고 완료 API 몫으로 남김. `TaskClientController`(`GET /api/v1/wms/tasks`, `POST /start`, `POST /complete`) 신규. api-spec도 동시 작업 — `models/tasks.dto.tsp`/`routes/client/tasks.api.tsp` 신규(main.tsp에 import+tagMetadata 추가), 예전에 써뒀던 stale한 피킹 placeholder(`PickingTaskResponse`/`PickingTaskAssignRequest`/`PickItemRequest`/`OutboundItemResponse`와 `outbound.api.tsp`의 `getPickingTasks`/`assignPickingTask`/`pickItem`)는 실제 Task 설계와 안 맞아 제거 | 완료 | 실제 서버(8096)로 전체 시나리오 검증: 재고 시딩→주문→FEFO 즉시 ALLOCATED→Task 자동 생성 확인 → `GET .../tasks?status=PENDING`으로 조회 → `POST .../start`(workerId=501)로 시작(응답 IN_PROGRESS, 전표도 PICKING으로 동시 전이 확인) → 이미 IN_PROGRESS인 Task 재시작 시도 409(WMS4096) 확인 → 다른 workerId(999)로 complete 시도 409(WMS4097, TASK_WORKER_MISMATCH) 확인 → 올바른 workerId(501)로 complete 성공(OutboundItem 5/5 PICKED, 전표 PACKING 전이, Inventory.quantity/reservedQuantity는 그대로인 것 확인) → 이미 COMPLETED인 Task 재완료 시도 409 확인. `npx tsp compile` 통과, `./gradlew :common:build :wms-service:build` 통과, 테스트 데이터 정리(DB 행 삭제, 서버 PID 종료). erd-spec 11절에 API/상태 롤업/재고 미차감 설명 3단락 추가, 8절 OutboundAllocatedEvent 설명 갱신, 논의사항 9번을 API 구현 완료 상태로 갱신 |
| 2026-09-25 | 사용자가 제시한 5단계 출고 플로우의 마지막 단계(최종 출고 완료 API — 진짜 quantity 차감 + RabbitMQ 완료 이벤트 발행)를 계획 후 구현. `POST /api/v1/wms/outbounds/complete`(`OutboundCompleteRequest{outboundOrderId}` — StockMovement/Task confirm과 동일한 body-id 컨벤션) 신규. `OutboundOrderService.completeShipment()`: `OutboundOrderJpaRepository.findWithPessimisticLockById()`(신규 — 지금까지 OutboundOrder엔 잠금 조회가 없었음)로 전표를 잠그고 `PACKING` 상태 확인(아니면 409 `INVALID_OUTBOUND_ORDER_STATUS`/WMS4098) — 성공하면 COMPLETED로 바뀌므로 재호출은 그냥 409로 막혀 별도 idempotency 키가 필요 없다(StockMovement의 `findPendingMovement()`와 동일 발상). `PICKED` 품목마다 `Inventory`를 찾아 `release()`→`remove()`로 실제 차감(`StockMovementQueryService.moveInventory()`와 동일 순서, 지금까지는 예약만 걸려 있고 물리 재고는 그대로였음). `order.complete()` 후 `OutboxService.recordOutboundCompleted()`(신규, `recordInboundCompleted()`와 동일 패턴)로 `wms.outbound.completed` 이벤트를 같은 트랜잭션에 기록. 이벤트 페이로드는 주문(orderId) 단위 완료 신호로 우선 정의 — `OutboundItem`에 아직 `orderItemId` 대응 컬럼이 없어(propose 단계 필드) 품목별 상세는 `{productId, quantity}`만 담음. `OutboundOrderResponse`(신규, id/orderId/warehouseId/status/createdAt) 응답. api-spec에도 동시 작업 — `models/outbound.dto.tsp`에 `OutboundOrderStatus`/`OutboundOrderResponse`/`OutboundCompleteRequest` 추가, `routes/client/outbound.api.tsp`에 `completeOutbound` 라우트 추가, `models/events.dto.tsp`/`routes/events/outbound.events.tsp`(신규)에 `OutboundCompletedEvent` 계약 추가(`inbound.events.tsp`와 동일 패턴, order-service 미구현 상태임을 명시) | 완료 | 실제 서버(8096)로 전체 폐루프 검증: 재고 시딩→주문(4개)→FEFO 즉시 ALLOCATED→Task 생성→start→complete(품목 PICKED, 전표 PACKING) → `POST .../outbounds/complete` 호출 → 응답 COMPLETED 확인, `inventory.quantity` 10→6(정확히 4 차감)·`reserved_quantity` 4→0 확인(가장 중요한 시나리오 — 예약만 걸려 있던 재고가 실제로 빠져나감), `wms_outbox`에 `wms.outbound.completed` 행이 정확한 페이로드(orderId=OMS 주문번호, items:[{productId,quantity}])로 기록된 것 확인, 이미 COMPLETED인 전표 재호출 시 409(WMS4098) 확인. `npx tsp compile`/`npm run lint`(신규 스키마 오펀 경고만 추가, 기존 패턴과 동일) 통과, `./gradlew :common:build :wms-service:build` 통과, 테스트 데이터 정리(DB 행 삭제, 서버 PID 종료). erd-spec 8절에 "출고 완료(실물 재고 차감)" 단락 신설, 11절 재고 미차감 설명에서 이 API로 링크, 논의사항 9번을 5단계 플로우 전체 완료 상태로 갱신 |
| 2026-09-25 | `feat/wms-outbound-flow#64`를 `develop`에 병합할 준비 — `origin/develop`을 로컬 브랜치에 병합(develop 쪽 15개 커밋이 order/product/auth/oms/infra만 건드리고 이 브랜치는 `services/wms-service`만 건드려 파일 충돌 0건, `git merge --strategy ort`로 클린 병합). 병합 준비 중 이 기능 전체를 무력화하는 문제를 직접 대조해 확인 — order-service의 실제 발행 클래스(`OrderInventoryConfirmEvent`)엔 `eventId`/`reservationToken`/`orderId`/`memberId`/`items(productId,quantity)`/`occurredAt`만 있고, WMS가 propose 단계로 가정해뒀던 `warehouseId`/`recipientInfo`/`orderItemId`/`productName`이 전혀 없다. 익스체인지/라우팅키는 일치해 메시지는 받지만 `createFromOrderEvent()`가 `event.warehouseId()`로 바로 창고를 조회해서, 이 값이 없으면 매번 `EntityNotFoundException`으로 DLQ行. 사용자에게 3가지 선택지(머지 보류/일단 머지 후 별도 이슈/WMS 쪽 임시 흡수)를 물어 "일단 develop에 머지, 스키마는 별도 이슈로" 확인받음 — erd-spec 논의사항에 11번(🔴 블로커로 표시)으로 기록, order-service 팀과 `warehouseId` 확정이 남은 실제 작업 | 완료(병합 준비) | `./gradlew :common:build :wms-service:build`(develop이 바꾼 `common/ApiResponse.java` 등 포함) 통과, `npx tsp compile` 통과. 아직 `origin`에 push는 안 함 — 사용자 확인 후 진행 예정. erd-spec 논의사항 11번 신설(🔴 블로커 표기) |
