...
**재고 실물 차감은 이 단계에서 하지 않는다**: `complete()`는 Task/OutboundItem/OutboundOrder 상태만 바꾸고 `Inventory.quantity`/`reserved_quantity`는 그대로 둔다 — "진짜 quantity 차감"은 아직 만들지 않은 최종 출고 완료 API(패킹 검수 후 실제로 창고를 떠나는 시점)의 몫으로 남겨뒀다.
...
9. (해결됨) **OutboundAllocatedEvent 소비자(피킹 Task 자동 생성) + 작업자 배정/시작/완료 API**: `OutboundReallocationListener.onOutboundAllocated()` → `TaskService.createPickingTasks()`로 Task 자동 생성 구현 완료(FEFO 분할 할당된 주문은 그만큼 여러 Task 생성, uk_task_outbound_item_id로 중복 방지). `TaskClientController`(`GET /api/v1/wms/tasks`, `POST /start`, `POST /complete`)로 조회/시작/완료 API도 구현 완료(11절 참고) — 별도 "배정" API 없이 `start` 요청의 `workerId`로 배정을 겸한다. 이번 버전은 부분 피킹(shortage)을 지원하지 않고 항상 전체 수량을 피킹한 것으로 기록한다. 아직 없는 것: (a) `task_no` 일자별 리셋 카운터(지금은 전역 시퀀스 기반 단순 채번), (b) 부분 피킹/SHORTAGE 지원, (c) 최종 출고 완료(실물 quantity 차감 + RabbitMQ 완료 이벤트 발행) API. 전부 다음 작업.
