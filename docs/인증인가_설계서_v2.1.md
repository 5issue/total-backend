...
- **전송 구간**: 외부 구간은 **ALB에서 ACM 인증서로 TLS 종료**. ALB 이후 내부 클러스터 구간은 프라이빗 VPC + NetworkPolicy로 통제하며 **재암호화하지 않는다**(0장 인프라 환경·3.2와 동일한 확정 사항).
  - **잔여 위험(수용)**: 동기 호출이 access token을 헤더로 전파하므로(3.4), 클러스터 내부 트래픽을 관측할 수 있는 주체에게는 bearer token과 개인정보가 평문으로 보인다. 내부 mTLS(서비스 메시) 도입은 인프라팀 결정 사항으로 남긴다.
